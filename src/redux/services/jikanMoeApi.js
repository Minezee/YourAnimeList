...
export const jikanMoeApi = createApi({
    ...
    endpoints: (builder) => ({
        getTopAnime: builder.query({query: () => '/top/anime?order_by=score&sort=desc'}),
        ...
        getTopAnimePage: builder.query({query: ({type , page}) => `/top/anime?order_by=score&sort=desc&page=${page}&sfw=false&type=${type}`}),
        ...
        getAnimeSearch: builder.query({query: ({type , query}) => `/anime?q=${query}&type=${type}&min_score=5.7&order_by=rank&sfw=false`}),
        getTopManga: builder.query({query: (page) => `/top/manga?page=${page}&order_by=score&sort=desc`}),
